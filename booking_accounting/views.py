...
from booking_accounting.serializer import BookingSerializer
...
class BookingViews(viewsets.ViewSet):
    ...
    def update(self,request,pk=None):
        ...
        serializer = ScheduleSerializer(obj,request.data,context={'request':request})
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response({'msg': 'Data  created', 'data':serializer.data}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    bookperam=OpenApiParameter(name='booking_date',description='Booking date',required=False,type=str,location=OpenApiParameter.QUERY)
